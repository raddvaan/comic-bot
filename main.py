...
class AppWindow:
        ...
        for item in pathlist:
            ...
            if dircheck:
                ...
                if filestatus == 'clear':
                    ...
                    year,
                    publisher,
                    filetype = self.name_scrub(item, filetype)
    ...
    def custom_edit_title(self,tree):
        ...
        self.vardict = {"self.seriesvar":self.seriescheck,
        ...
        self.sissuestv = ttk.Treeview(self.lbframe,yscrollcommand=self.ilbsb.set)
        self.sissuestv.heading("#0",text="New Name",command=lambda:self.tv_sort("edit","#0","reve1"))
...
#
        ...
        self.seriesCb = Checkbutton(self.infof,bd=1,variable=self.seriescheck)
        ...
        self.volumeCb = Checkbutton(self.infof,bd=1,variable=self.volumecheck)
        ...
        self.publisherCb = Checkbutton(self.infof,bd=1,variable=self.pubcheck)
        ...
        self.subtitleCb = Checkbutton(self.infof,bd=1,variable=self.subcheck,command=self.print_values)
...
#
        ...
        self.updateB = Button(self.infof,text="Update",command=self.update_preview)
    ...
    def populate_edit_screen(self,selected,tree):
        ...
        self.nextb.config(command=lambda:self.browse_buttons(firstiss,"next",tree))
        self.prevb.config(command=lambda:self.browse_buttons(firstiss,"prev",tree))
        tagtuple = self.get_tags(firstiss)
        curissue, curseries, curvol, curevent, curyear, curpub,cursub = tagtuple
        ...
        for key,value in self.issuedict.items():
            ...
            if i_sub != cursub:
                cursub = ""
                self.subcheck.set(0)
            additem = self.sissuestv.insert("",'end',text=value,iid=key,tags=self.newtree.item(key,"tags"))
        ...
        #self.sissueslb.itemconfig(0,text="ass")
        ...
        for key,value in self.vardict.items():
            ...
            if key == "":
                value.set(0)
        self.saveB.config(command=lambda:self.save_edits(iid,pid,self.issuedict))
    ...
    def update_preview(self):
        ...
        for item in stree.get_children(""):
            ...
            newTags = ("issue",issueCheckD[self.issuecheck.get()],
            ...
            newname = self.name_builder(newTags[3],newTags[13],newTags[5],newTags[1],newTags[9],newTags[15])
    ...
    def save_edits(self,iid,pid,issuedict):
        ...
        for item in stree.get_children(""):
            ...
            newtree.item(item,tags=stree.item(item,"tags"),text=stree.item(item,"text"))
            self.move_item(item)
...
##########################################################
    ...
    def name_scrub(self,dname,filetype): 
        ...
        return dname, fname, title, subtitle, issue, evtitle, fvol,issyr,"",filetype
    ...
    def event_handler(self,dname):
        ...
        eviss = re.search("\A\d{1,3}(\.\d{1,2})?[\s]-?",dname)#.group(0)
